...
function Tile(obj, x, y, tileData) {
	...
	var $img = $("<img class='floor' src='/resource/game/images/tile/base_floor/groundtiles_tile"+getNum(this.x, this.y)+".png'>").appendTo(this.obj);
...
Tile.prototype.renderConstruction = function(level) {
	if(level < 5) {
		for(var i=1; i<=level; i++) {
			if(i < level) {
				var $img = $("<img class='building lv"+i+"' src='/resource/game/images/building/"+getAreaTypeName(this.area_type)+"_Lv1.png'/>").appendTo(this.obj);
				$img.css("z-index", (this.x+this.y)*10+1+i);
			} else {
				var $img = $("<img class='construction building lv"+i+"' src='/resource/game/images/building/construction.png'/>").appendTo(this.obj);
				$img.css("z-index", (this.x+this.y)*10+1+i);
			}
		}
		...
		var $img = $("<img class='construction building lv"+level+"' src='/resource/game/images/building/construction.png'/>").appendTo(this.obj);
...
Tile.prototype.renderBuilding = function(level, no_effect) {
	if(level < 5) {
		...
		var $img = $("<img class='building lv"+level+"' src='/resource/game/images/building/"+getAreaTypeName(this.area_type)+"_Lv5.png'/>").appendTo(this.obj);
		...
		var $img = $("<img class='building lv"+level+"' src='/resource/game/images/tile/"+getAreaTypeName(this.area_type)+"_LvFLETA-Tile.png' style='opacity:1'/>").appendTo(this.obj);
		$img.css("z-index", (this.x+this.y)*10+level);
		var $img = $("<img class='building lv"+level+"' src='/resource/game/images/building/"+getAreaTypeName(this.area_type)+"_LvFLETA.png'/>").appendTo(this.obj);
		var zindex = (this.x+this.y)*10+1+level
		$img.css("z-index", zindex);
		this.obj.css("z-index", (this.x+this.y)*10-1);
		if(!no_effect) {
