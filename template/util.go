...
func extractPage(n *html.Node, m map[string][]byte) {
	for n != nil {
...
func callMethod(method reflect.Value, r *http.Request) (map[string][]byte, error) {
	ret1 := method.
		Call(
			[]reflect.Value{
				reflect.ValueOf(r),
...
func isContollerMethod(method reflect.Value) bool {
	...
	if mapParam.String() != "map[string][]uint8" {
