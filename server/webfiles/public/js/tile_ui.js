...
function newObjDiv(x,y,num) {
	return $("<div/>")
		.css("z-index", (+x+ +y)*10)
		.append($("<img class='floor' src='/public/images/tile/base_floor/groundtiles_tile"+num+".png'>"))
}
...
TileUI.prototype.BuildUp = function() {
	...
	this.obj.find("img.floor").attr("src", "/public/images/tile/building_floor.png");
	if (lv < 5) {
		for (var i = 0 ; i < lv ; i++) {
			if (i == lv-1) {
				var $img = $("<img class='building lv"+(lv)+"' src='/public/images/building/construction.png'/>")
			} else {
				var $img = $("<img class='building lv"+(i+1)+"' src='/public/images/building/"+this.Tile.TypeName()+"_Lv1.png'/>")
			}
			this.obj.append($img);
		}
	} else if (lv == 5) {
		this.obj.find(".building").detach();
		var $img = $("<img class='building lv5' src='/public/images/building/construction.png'/>")
		this.obj.append($img);
	} else if (lv == 6) {
		...
		if (checker.CheckLvF(function (t, headT) {
			...
			var $img = $("<img class='building lv6' src='/public/images/building/construction.png'/>")
...
TileUI.prototype.completBuilding = function (effect) {
	...
	if (lv == 6) {
		...
		this.obj.find("img.floor").attr("src", "/public/images/tile/"+this.Tile.TypeName()+"_LvFLETA-Tile.png").addClass("lv6");
	...
	this.obj.find(".lv"+lv+".building").attr("src", "/public/images/building/"+this.Tile.TypeName()+""+fileTail+".png")
	if (effect != "noEffect") {
...
TileUI.prototype.Remove = function() {
	...
	this.obj.find(".floor").attr("src", "/public/images/tile/base_floor/groundtiles_tile"+this.num+".png").attr("class", "floor");
